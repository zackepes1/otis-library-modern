...
export const metadata: Metadata = {
  title: "O'tis a Festival | Otis Library",
  description: "O'tis a Festival — Otis Library's annual community celebration with activities, entertainment, and fun for all ages.",
};

export default async function OtisFestivalPage() {
  ...
  try {
    ...
    upcoming = all.filter(
      (e) => e.date >= todayStr && e.title.toLowerCase().includes("o'tis"),
    );
  } catch {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-12 sm:px-6 sm:py-16">
      ...
      <h1 className="mt-2 text-3xl font-semibold text-white">O&apos;tis a Festival</h1>
      <p className="mt-3 text-slate-400">
        Otis Library's annual community celebration — activities, entertainment, and fun for all ages.
      </p>

      <div className="mt-10">
        {upcoming.length === 0 ? (
          <div className="rounded-xl border border-white/10 bg-white/[0.03] p-8 text-center">
            <p className="text-slate-400">No upcoming dates are listed yet — check back closer to the event.</p>
            <a
