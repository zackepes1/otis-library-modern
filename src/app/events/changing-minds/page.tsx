...
export const metadata: Metadata = {
  title: "Changing Minds Book Club | Otis Library",
  description: "Upcoming dates for the Changing Minds Book Club at Otis Library — a recurring discussion group exploring thought-provoking fiction and nonfiction.",
};

export default async function ChangingMindsPage() {
  ...
  try {
    ...
    upcoming = all.filter(
      (e) => e.date >= todayStr && e.title.toLowerCase().includes("changing minds"),
    );
  } catch {
    // fall through to empty state
  }

  // Single upcoming occurrence — go straight to the detail page
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-12 sm:px-6 sm:py-16">
      ...
      <h1 className="mt-2 text-3xl font-semibold text-white">Changing Minds Book Club</h1>
      <p className="mt-3 text-slate-400">
        A recurring discussion group exploring thought-provoking fiction and nonfiction titles.
      </p>

      <div className="mt-10">
        {upcoming.length === 0 ? (
          <div className="rounded-xl border border-white/10 bg-white/[0.03] p-8 text-center">
            <p className="text-slate-400">No upcoming dates are listed yet.</p>
            <a
